// borrowed from github.com/gdamore/tcell/views/cellarea.go
...
func (a *CellView) Draw() {
	...
	a.port.SetContentSize(ex, ey, true)
	vx, vy := port.Size()
	if ex < vx {
		ex = vx
	}
...
func (a *CellView) SetModel(model views.CellModel) {
	w, h := model.GetBounds()
	model.SetCursor(0, 0)
	a.model = model
...
func (a *CellView) SetView(view views.View) {
	port := a.port
	port.SetView(view)
	a.view = view
	if view == nil {
